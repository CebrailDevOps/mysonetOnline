...
<?php
    ...
    // Insérez le pseudo, l'IP, la date/heure et une référence dans le fichier demandes_en_attente
        ...
        include 'ssh.php';
        $ssh_command = 'ssh -i '.$ssh_path.' -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null inspectorsonet@' . $secure_ami_ip . ' ' . escapeshellarg($command);
        shell_exec($ssh_command);

        // Lire la dernière ligne du fichier demandes_en_attente
        $last_line_command = 'ssh -i '.$ssh_path.' -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null inspectorsonet@' . $secure_ami_ip . ' tail -n 1 /home/inspectorsonet/demandes_en_attente';
    ...
    $ssh_command = 'ssh -i '.$ssh_path.' -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null inspectorsonet@' . $_SESSION['ip'] . ' ' . escapeshellarg($command);
    shell_exec($ssh_command);
