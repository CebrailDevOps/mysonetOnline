<?php
    ...
    foreach($demandes as $demande) {
        ...
        if($result == 0) {
            ...
            include 'ssh.php';
            $command = 'echo "' . $demande['ref_demande'] . ';' . $pseudo_demandeur . ';' . $ip_demandeur . ';' . $date . '" | ssh -i '.$ssh_path.' -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null inspectorsonet@' . escapeshellarg($ip) . ' "cat >> /home/inspectorsonet/demandes_en_attente"';

            // Exécuter la commande SSH
            shell_exec($command);

            // Lire la dernière ligne du fichier demandes_en_attente
            $last_line_command = 'ssh -i '.$ssh_path.' -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null inspectorsonet@' . escapeshellarg($ip) . ' "tail -n 1 /home/inspectorsonet/demandes_en_attente"';
